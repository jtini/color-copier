...
if (rgbPref === 'on' || hsvPref === 'on' || hslPref === 'on') {
...
if (figma.command === 'set-preferences') {
    ...
    if (
        ...
        const paints = layer.type === 'FRAME' ? layer.backgrounds : layer.type === 'LINE' ? layer.strokes : layer.fills;
        ...
        if (paints !== figma.mixed && paints.length === 1) {
            ...
            if (fill.type !== 'SOLID') {
                figma.notify(
                    `Please select a ${layer.type === 'FRAME' ? 'frame' : 'layer'} with a color ${
                        layer.type === 'FRAME' ? 'background' : 'fill'
                    }.`
                );
                figma.closePlugin();
            } else {
            ...
            switch (figma.command) {
                case 'copy': {
                    ...
                    if (
                        ...
                        if (hexPref === 'on') {
                            str += rgbPref === 'on' ? '\n' + baseColor.toHexString() : baseColor.toHexString();
                        }
                        if (hsvPref === 'on') {
                            str +=
                                hexPref === 'on' || rgbPref === 'on'
                                    ? '\n' + baseColor.toHsvString()
                                    : baseColor.toHsvString();
                        }
                        if (hslPref === 'on') {
                            str +=
                                hsvPref === 'on' || hexPref === 'on' || rgbPref === 'on'
            ...
            figma.notify(
                `Please select a ${layer.type === 'FRAME' ? 'frame' : 'layer'} with one ${
                    layer.type === 'FRAME' ? 'background' : 'fill'
                }.`
            );
            figma.closePlugin();
...
figma.ui.onmessage = msg => {
    if (msg.type === 'set-preferences') {
        if (!msg.data.rgb && !msg.data.hex && !msg.data.hsv && !msg.data.hsl) {
            figma.notify('Please select at least one format');
        } else {
            if (msg.data && msg.data.rgb) {
                figma.root.setPluginData('rgb', msg.data.rgb === 'off' ? 'off' : 'on');
            }
            if (msg.data && msg.data.hex) {
                figma.root.setPluginData('hex', msg.data.hex === 'off' ? 'off' : 'on');
            }
            if (msg.data && msg.data.hsv) {
                figma.root.setPluginData('hsv', msg.data.hsv === 'off' ? 'off' : 'on');
            }
            if (msg.data && msg.data.hsl) {
                figma.root.setPluginData('hsl', msg.data.hsl === 'off' ? 'off' : 'on');
            }
